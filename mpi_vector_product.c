...
void Parallel_vector_product( double local_x[], double local_y[],
      double local_z[],int local_n);
...
int main(void) {
   ...
   Parallel_vector_product(local_x, local_y, local_z, local_n);
   tend = MPI_Wtime();

   Print_vector(local_z, local_n, n, "\nThe product is", my_rank, comm);
   if(my_rank==0)
    printf("\nTook %f ms to run\n", (tend-tstart)*1000);
...
void Parallel_vector_product(
    ...
    for (local_i = 0; local_i < local_n; local_i++) {
        dot_product += local_x[local_i] * local_y[local_i];
